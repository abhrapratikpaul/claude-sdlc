...
- Name: file-validation.spec.ts >> FR-002: File Type Validation >> AC-003: System checks file extension matches .pdf (case-insensitive)
- Location: tests\file-validation.spec.ts:95:7
...
```ts
  ...
  4   | 
  5   | /**
  6   |  * Test Suite: File Type and Size Validation
  7   |  * Covers: AC-003, AC-004, AC-005 (FR-002), AC-006, AC-007 (FR-003)
  8   |  */
  9   | 
  10  | test.describe('FR-002: File Type Validation', () => {
  11  | 
  12  |   test.beforeAll(() => {
  13  |     // Create fixtures directory if it doesn't exist
  14  |     const fixturesDir = path.join(__dirname, '../fixtures');
  15  |     if (!fs.existsSync(fixturesDir)) {
  16  |       fs.mkdirSync(fixturesDir, { recursive: true });
  17  |     }
  18  | 
